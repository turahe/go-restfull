...
func (m *mockMediaService) GetTree(ctx context.Context, actorUserID uint) ([]service.MediaTreeNode, error) {
	args := m.Called(ctx, actorUserID)
	return args.Get(0).([]service.MediaTreeNode), args.Error(1)
}
func (m *mockMediaService) GetSubtree(ctx context.Context, actorUserID uint, mediaID uint) ([]service.MediaTreeNode, error) {
